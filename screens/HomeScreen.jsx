...
const HomeScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await fetch(`http://192.168.56.183:5000/api/auth/user?email=${email}`);
  ...
  useEffect(() => {
    const fetchAllDoctors = async () => {
      try {
        const res = await fetch(`http://192.168.56.183:5000/api/doctors`);
        const data = await res.json();
        if (res.ok) {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        const res = await fetch(`http://192.168.56.183:5000/api/bookings?email=${user.email}`);
        const data = await res.json();
